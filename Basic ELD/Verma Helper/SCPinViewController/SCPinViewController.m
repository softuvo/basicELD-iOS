...
- (void)viewDidLoad {
 ...
 //   [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(ReloadPAsscodeView) name:kReloadPAsscodeView object:nil];
}
    -(void)ReloadPAsscodeView{
        self.mLogoutButton.hidden = true;
    }
...
- (void)wrongPin {
    ...
    dispatch_after(delayInSeconds, dispatch_get_main_queue(), ^(void){
        ...
        if ([strongSelf.validateDelegate respondsToSelector:@selector(pinViewControllerDidSetWrongPin:)]) {
            ...
            [self.KeybordVIew.layer addAnimation:shake forKey:@"shake"];
        }
...
-(void)LockAnimation{
    if(imageRotation==0)
    {
        [UIImageView animateWithDuration:0.5 animations:^{
            self.LockInnnerImageView.transform = CGAffineTransformMakeRotation(.34906585);
            self.LockOuterImageView.transform = CGAffineTransformMakeRotation(1.34906585);
        }];
        imageRotation=1;
    }
    else if (imageRotation==1)
    {
        [UIImageView animateWithDuration:0.5 animations:^{
            self.LockInnnerImageView.transform=CGAffineTransformMakeRotation(M_PI / 2);
            self.LockOuterImageView.transform=CGAffineTransformMakeRotation( 7.34906585);
        }];
        imageRotation=2;
    }
    ...
    {
        [UIImageView animateWithDuration:0.5 animations:^{
            self.LockInnnerImageView.transform = CGAffineTransformMakeRotation((M_PI / 3));
            
            self.LockOuterImageView.transform = CGAffineTransformMakeRotation(3.34906585);
            
            
        }];
        imageRotation=3;
    }
    else if (imageRotation==3)
    {
        [UIImageView animateWithDuration:0.5 animations:^{
            self.LockInnnerImageView.transform = CGAffineTransformMakeRotation(0);
            
            self.LockOuterImageView.transform = CGAffineTransformMakeRotation(0);
...
-(IBAction)LogoutButtonAction:(id)sender{
    ...
    UIAlertView *alrt = [[UIAlertView alloc] initWithTitle:@"\nForgot your Passcode?" message:@"Tap ‘OK’ to log out, when you log in back you’ll be able to create a new Passcode.\n\nIf you remember your Passcode, tap ‘Cancel’ to go back" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"OK", nil];
    alrt.tag = 1001;
    [alrt show];
    alrt = nil;
}
